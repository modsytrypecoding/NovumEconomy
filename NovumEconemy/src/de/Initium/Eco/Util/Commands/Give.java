                        ...
                        if(!t.isOnline()) {
                            if(Main.playerBank.containsKey(t.getUniqueId())) {
                                if(Double.parseDouble(args[1]) > Integer.MAX_VALUE) {
                                    p.sendMessage(MainDis.InputNumberToBig); //config
                                }else {
                                    Main.getImplementer().depositPlayer(t, Double.parseDouble(args[1]));
                                    p.sendMessage(MainDis.MoneyGiveSuccess.replace("%target%", t.getName()).replace("%value%", args[1]).replace("%currency%", MainDis.CurName));
                                }
