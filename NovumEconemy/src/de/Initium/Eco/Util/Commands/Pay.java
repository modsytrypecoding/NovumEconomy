                        ...
                        if(Main.playerBank.containsKey(t.getUniqueId())) {
                            if(!t.isOnline()) {
                                if(Double.parseDouble(args[1]) > Main.getImplementer().getBalance(p)) {
                                    p.sendMessage(MainDis.BalanceToLow); //config
                                }else {
                                    ...
                                    p.sendMessage(MainDis.MoneyPaySuccess.replace("%target%", t.getName()).replace("%value%", args[1]).replace("%currency%", MainDis.CurName));
                                }
                            }else {
                                ...
                                if(Double.parseDouble(args[1]) > Main.getImplementer().getBalance(p)) {
                                    ...
                                    p.sendMessage(MainDis.MoneyReceiveSuccess.replace("%player%", p.getName()).replace("%value%", args[1]).replace("%currency%", MainDis.CurName));
                                }
                            }
                        }else {
